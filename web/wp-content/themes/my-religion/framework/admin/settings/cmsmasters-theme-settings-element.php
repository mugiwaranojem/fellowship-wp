...
function my_religion_options_element_fields($set_tab = false) {
	...
	case 'code':
		...
		$options[] = array( 
			'section' => 'code_section', 
			'id' => 'my-religion' . '_twitter_access_data', 
			'title' => esc_html__('Twitter Access Data', 'my-religion'), 
			'desc' => sprintf(
				/* translators: Twitter access data. %s: Link to twitter access data generator */
				esc_html__( 'Generate %s and paste access data to fields.', 'my-religion' ),
				'<a href="' . esc_url( 'https://api.cmsmasters.net/wp-json/cmsmasters-api/v1/twitter-request-token' ) . '" target="_blank">' .
					esc_html__( 'twitter access data', 'my-religion' ) .
				'</a>'
			), 
			'type' => 'multi-text', 
			'std' => array(),
			'class' => 'regular-text', 
			'choices' => array( 
				esc_html__('Consumer Key', 'my-religion') . '|consumer_key', 
				esc_html__('Consumer Secret', 'my-religion') . '|consumer_secret', 
				esc_html__('Access Token', 'my-religion') . '|access_token', 
				esc_html__('Access Token Secret', 'my-religion') . '|access_token_secret' 
			) 
		);
