...
function my_religion_get_admin_page() {
	global $pagenow;
...
function my_religion_default_tab() {
	$current_page = my_religion_get_admin_page();
...
function my_religion_get_the_tab() {
	global $pagenow;
	
	$default_tab = my_religion_default_tab();
...
function my_religion_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'my-religion') . '</a>';
		}
